...
public class Line {
    ...
    public Line(Point p1, Point p2) {
        if (areEquivalent(p1.x, p2.x)) {
            ...
            y_intercept   = p1.y - slope * p1.x; // to find y_intercept, we plugged in 1 of the Points into equation of line
            if (y_intercept == 0) {
                x_intercept = 0;
            } else {
    ...
    public static boolean areEquivalent(double a, double b) {
        return Math.abs(a - b) < 0.0001; // Can't use == for doubles. Use epsilon = 0.0001
    }
